"""
performance.py  —  behavior run2
---------------------------------
Per-task learning curves for the combined run0 + run1 + run2 behavioural
dataset (80 tasks).  Produces three figures with increasing levels of
filtering:
...
Usage:
    cd <repo root>
    python experiments/behavior/run2/performance.py
"""
...
# ---------------------------------------------------------------------------
# Load + concatenate raw data (run0 + run1 + run2)
# ---------------------------------------------------------------------------
print(f"Reading {cfg.BEHAVIOR_CSV_RUN0.name} + "
      f"{cfg.BEHAVIOR_CSV_RUN1.name} + "
      f"{cfg.BEHAVIOR_CSV_RUN2.name}")
df_raw = pd.concat(
