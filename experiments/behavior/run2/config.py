"""
Configuration for behavior run2.

The full dataset concatenates run0, run1, and run2 for a combined corpus
covering the same 80 tasks.  run2 adds further assignments for a subset of
those tasks; no new tasks are introduced.
"""
...
RESOURCES  = Path(__file__).parent / "ressources"

# All three CSVs are concatenated at load time (no overlapping assignments)
BEHAVIOR_CSV_RUN0 = RESOURCES / "dlbt-behavior.csv"
BEHAVIOR_CSV_RUN1 = RESOURCES / "dlbt-behavior-run1.csv"
BEHAVIOR_CSV_RUN2 = RESOURCES / "dlbt-behavior-run2.csv"
...
# ---------------------------------------------------------------------------
# Full BEH_ID_TO_TASK mapping — all 80 tasks across run0 + run1 + run2
