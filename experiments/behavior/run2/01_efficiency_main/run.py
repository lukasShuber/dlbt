"""
run2/01_efficiency_main/run.py — trials-per-task budget sweep.

Protocol
--------
1.  Load + filter run0 + run1 + run2 data; identify eligible tasks.
...
6.  Save summary dict as results/efficiency_main_01_dlbt_frozen_slda_frozen.pkl.
...
def _all_data_ds(tasks: list, flip: bool = False) -> BehavioralDataset:
    """Build a training dataset from the fixed 90 % pool."""
    all_trials = []
    for task_name in tasks:
...
def _run_dlbt(
    ...
    """Train DLBT mapper (Phase 1 only — encoder frozen)."""
    train_dlbt(
    ...
    if not cfg.FREEZE_ENCODER_DLBT:
        # Phase 2 — attnpool fine-tuning (disabled in this config)
...
def _run_slda(
    ...
    """Fit SLDA logistic regression (Phase 1 only — encoder frozen)."""
